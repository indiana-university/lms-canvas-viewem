...
Insert a record into the `LTI_13_AUTHZ` table with your tool's registration_id (`lms_lti_viewem`), along with the client_id 
...
<tr><td>Initiate login URL</td><td>http://localhost:8080/lti/login_initiation/lms_lti_viewem</td></tr>
...
| `canvas.oauth2.clientId.viewem`     |                             | Client ID for the Canvas Developer Key used for OAuth2 authentication     |
| `canvas.oauth2.clientSecret.viewem` |                             | Client secret for the Canvas Developer Key used for OAuth2 authentication |
...
```
Redirect URI should be `<host>/login/oauth2/code/lms_canvas_oauth2_viewem`
